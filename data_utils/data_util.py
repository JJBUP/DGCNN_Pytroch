...
# class id 2 label标签的字典 {id:a,id:b,id:c...}
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    """ Convert original dataset files to data_label file (each line is XYZRGBL).
        ...
        将原始数据集文件转换为data_label文件（每行为XYZRGB+Label）。
        我们汇总了房间中每个实例的所有点。
    Args:
        ...
        anno_path: 注释文件的绝对路径
        out_filename: 修正后保存文件的路径，文件中每一行为 XYZRGB+LABEL
        file_format: txt格式或numpy格式，其为保存格式
    Returns:
        None
    Note:

        the points are shifted before save, the most negative point is now at origin.
        注意：保存之前，点会移动，最不利的点现在位于原点。
    """
    points_list = []
    # glob 使用通配符获得当前路径下匹配的路径
    # 如： p="/a/*.txt"  glob.glob(p) --> /a/1.txt /a/2.txt...
    for f in tqdm(glob.glob(os.path.join(anno_path, '*.txt'))):
        # 获得分类数量
        # basenemae 去掉.txt split获得按通配符分割后的名称，如：/a/ceiling_2.txt-->ceiling
        # print("正在处理的文件: ", f)
        cls = os.path.basename(f).split('_')[0]
        if cls not in g_classes:  # note: in some room there is 'staris' class..
            cls = 'clutter'  # 分类中如果不需要或没有对该物体分类，将其归为杂物cluster

        points = np.loadtxt(f)  # Annotations中的txt，该txt为xyzrgb，label 为txt名字
        ...
        # 拼接 point和label  xyzrgb+label
        points_list.append(np.concatenate([points, labels], axis=1))  # Nx7

    # 将一个area中的一个数据分割的部分拼接成一个完整的如：conferenceRoom_1=(chair+board...)
