...
def parse_args():
    ...
    # todo: 根据GPU数目设置batch_size
    # batch_size大小
    parser.add_argument('--batch_size', type=int, default=24 * 2, help='Batch Size during training [default: 16]')
    # todo: 设置可选GPU
    parser.add_argument('--GPU_list', type=str, default="2,3", help='可用gpu编号,例如：0,2,3')
    # epoch
    parser.add_argument('--epoch', default=200, type=int, help='Epoch to run [default: 32]')
    # 线程数number_work
    parser.add_argument('--num_workers', type=int, default=12, help='dataloader work number')
    # 是否将数据驻留在内存中
    ...
    parser.add_argument('--resume_log_dir', type=str, default=None,
                        help='Log path [default: 2022-08-24_14-10-33-msg-dp]')
    # 优化器
...
def main(args):
    ...
    TRAIN_DATASET = RandomAllSceneDataset(is_train=True, data_root=args.dataset_root, block_points_num=NUM_POINT,
                                          scale=args.scale, test_area=args.test_area,
    ...
    label_weights = torch.Tensor(TRAIN_DATASET.labelweights).cuda()
    ...
    classifier = DataParallel(classifier.to(device), device_ids=gpu_id_list, output_device=gpu_id_list[0])
    # 优化器选择
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for points, target in tqdm(iterable=train_dataLoader, total=len(train_dataLoader),
                                   colour="BLUE", desc="epoch[{}/{}]-train".format(epoch + 1, args.epoch)):
            ...
            pred_choice_np = seg_pred.cpu().data.argmax(1).numpy()
            target_np = target.cpu().data.numpy()
            # 计算该batch 的正确率
            correct = np.sum(pred_choice_np == target_np)
            # 累计 的预测对的点正确个数
        ...
        with torch.no_grad():  # 强制之后的内容不进行计算图构建
            ...
            for points, target in tqdm(iterable=test_dataLoader, total=len(test_dataLoader),
                                       colour="BLUE", desc="epoch[{}/{}]-test".format(epoch + 1, args.epoch)):
                points = points.data.numpy()
                points = torch.Tensor(points)
                points, target = points.float().cuda(), target.long().cuda()
